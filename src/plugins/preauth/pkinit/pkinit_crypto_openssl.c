                              ...
                              krb5_algorithm_identifier ***algs_out)
{
    ...
    *algs_out = NULL;
    ...
    for (count = 0; supported_cms_signature_ids[count] != NULL; count++);
    algs = k5calloc(count + 1, sizeof(krb5_algorithm_identifier *), &ret);
    if (algs == NULL)
        goto cleanup;

    /* Add an algorithm identifiers for each OID, with no parameters. */
    for (i = 0; i < count; ++i) {
        algs[i] = k5alloc(sizeof(krb5_algorithm_identifier), &ret);
        if (algs[i] == NULL)
            goto cleanup;
        ret = krb5int_copy_data_contents(context,
                                         supported_cms_signature_ids[i],
                                         &algs[i]->algorithm);
        if (ret)
            goto cleanup;
